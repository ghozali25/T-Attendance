-- ================================================
-- T-Absensi Database Migration Script
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'attendance' AND column_name = 'clock_in_lat') THEN
    ALTER TABLE public.attendance ADD COLUMN clock_in_lat DOUBLE PRECISION DEFAULT NULL;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'attendance' AND column_name = 'clock_in_lng') THEN
    ALTER TABLE public.attendance ADD COLUMN clock_in_lng DOUBLE PRECISION DEFAULT NULL;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'attendance' AND column_name = 'clock_out_lat') THEN
    ALTER TABLE public.attendance ADD COLUMN clock_out_lat DOUBLE PRECISION DEFAULT NULL;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'attendance' AND column_name = 'clock_out_lng') THEN
    ALTER TABLE public.attendance ADD COLUMN clock_out_lng DOUBLE PRECISION DEFAULT NULL;
  END IF;
END $$;
...
DROP INDEX IF EXISTS idx_attendance_user_date_unique;
...
DROP POLICY IF EXISTS "Managers can view own audit logs" ON public.audit_logs;
